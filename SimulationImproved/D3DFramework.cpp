...
unique_ptr<D3DFramework> D3DFramework::_instance = make_unique<D3DFramework>();
...
#else
	{
		const string fileName{ "..\\Debug\\Simulation_VS.cso" };
		ifstream fin(fileName, ios::binary);
		if (!fin) {
			MessageBox(nullptr, L"The CSO file cannot be found.", L"Error", MB_OK);
			return E_FAIL;
		}
		vector<unsigned char> byteCode(istreambuf_iterator<char>(fin), {});
...
#else
	{
		const string fileName{ "..\\Debug\\Simulation_PS.cso" };
		ifstream fin(fileName, ios::binary);
		if (!fin) {
			MessageBox(nullptr, L"The CSO file cannot be found.", L"Error", MB_OK);
			return E_FAIL;
...
void D3DFramework::render() {
	...
	//
	_pImmediateContext->ClearRenderTargetView(_pRenderTargetView, Colors::LightGray);
